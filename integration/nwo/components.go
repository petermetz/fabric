...
import (
	...
	"github.com/petermetz/fabric/integration/helpers"
	"github.com/petermetz/fabric/integration/runner"
...
func (c *Components) Build(args ...string) {
	...
	cryptogen, err := gexec.Build("github.com/petermetz/fabric/common/tools/cryptogen", args...)
	Expect(err).NotTo(HaveOccurred())
	c.Paths["cryptogen"] = cryptogen

	idemixgen, err := gexec.Build("github.com/petermetz/fabric/common/tools/idemixgen", args...)
	Expect(err).NotTo(HaveOccurred())
	c.Paths["idemixgen"] = idemixgen

	configtxgen, err := gexec.Build("github.com/petermetz/fabric/common/tools/configtxgen", args...)
	Expect(err).NotTo(HaveOccurred())
	c.Paths["configtxgen"] = configtxgen

	orderer, err := gexec.Build("github.com/petermetz/fabric/orderer", args...)
	Expect(err).NotTo(HaveOccurred())
	c.Paths["orderer"] = orderer

	peer, err := gexec.Build("github.com/petermetz/fabric/peer", args...)
	Expect(err).NotTo(HaveOccurred())
	c.Paths["peer"] = peer

	discover, err := gexec.Build("github.com/petermetz/fabric/cmd/discover", args...)
	Expect(err).NotTo(HaveOccurred())
